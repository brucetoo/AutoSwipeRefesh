 ...
 * On 8/7/15.
 * At 11:42
 * All source code from support v4 SwipeRefreshLayout
 * just modify {@param mNotify}
 * when first reach one page use SwipeRefreshLayout,i may just want it to be refresh automatically
 * but former code need user pull to refresh
 * see code method {@link #setRefreshing(boolean)}
 * and set mNotify = true;
 * Then u see {@param mRefreshListener}
 * if (mNotify) {  //make callback available here ,then u can auto refresh now
     if (mListener != null) {
    mListener.onRefresh();
    }
 }
